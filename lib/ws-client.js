/**
 * WebSocket Client
 * Connects to the OpenWeb daemon, handles reconnection, and routes messages.
 */

import { executeTool } from "../tools/registry.js";

// Callbacks set by background.js to avoid circular dynamic imports
// (dynamic import of background.js from ws-client.js crashes the Chrome service worker)
...
// ── Rate Limiting (self-contained, no circular import) ────────────────────────
let _rateLimitPerSec = 0;
const _callTimestamps = [];

// Restore rate limit from storage on service worker wake-up
...
function checkRateLimit() {
  if (_rateLimitPerSec <= 0) return true; // unlimited
  const now = Date.now();
  const windowMs = 1000;
